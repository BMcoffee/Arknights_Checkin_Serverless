...
function main_handler(){
    ...
    // 服务器设置
    $server = 'CN'; // US 美服|全球服 || JP 日服

    // 平台设置
    $GLOBALS['PLATFORM_ID'] = 'Android'; //'IOS'

    if ($server == 'JP'){
        // 日服
        $GLOBALS['HOST_PASSPORT_SERVER'] = 'https://passport.arknights.jp';
        $GLOBALS['HOST_AUTH_SERVER'] = 'https://as.arknights.jp';
        $GLOBALS['HOST_GAME_SERVER'] = 'https://gs.arknights.jp:8443';
        $GLOBALS['HOST_CONFIG_SERVER'] = 'https://ak-conf.arknights.jp';
        $GLOBALS['HOST_VERSION_ADDR'] = "https://ark-jp-static-online.yo-star.com/assetbundle/official/{$GLOBALS['PLATFORM_ID']}/version";
        $GLOBALS['SERVER_ID'] = '3';
    } elseif ($server == 'US'){
        // 美服|全球服
        $GLOBALS['HOST_PASSPORT_SERVER'] = 'https://passport.arknights.global';
        $GLOBALS['HOST_AUTH_SERVER'] = 'https://as.arknights.global';
        $GLOBALS['HOST_GAME_SERVER'] = 'https://gs.arknights.global:8443';
        $GLOBALS['HOST_CONFIG_SERVER'] = 'https://ak-conf.arknights.global';
        $GLOBALS['HOST_VERSION_ADDR'] = "https://ark-us-static-online.yo-star.com/assetbundle/official/{$GLOBALS['PLATFORM_ID']}/version";
        $GLOBALS['SERVER_ID'] = '3';
    } else {
        // 国服
        $GLOBALS['HOST_AUTH_SERVER'] = 'https://as.hypergryph.com';
        $GLOBALS['HOST_GAME_SERVER'] = 'https://ak-gs-gf.hypergryph.com';
        $GLOBALS['HOST_CONFIG_SERVER'] = 'https://ak-conf.hypergryph.com';
        $GLOBALS['HOST_VERSION_ADDR'] = "https://ak-conf.hypergryph.com/config/prod/official/{$GLOBALS['PLATFORM_ID']}/version";
    }
    updata_config();
    
    // 国际服
    //$player->set_yostar_login_token('yostarLoginToken');
    //$player->set_is_yostar(true);
    
    // Auth登录
    //$player->set_access_token('accessToken');
...
function sendEmails(){
    // 发送邮件使用的邮箱相关信息，第三方 SMTP 服务
    $mail_host = "XXXXX";         //邮箱服务器地址
    $mail_user = "XXX@XXX.XXX";    //发件人邮箱地址
    $mail_pass = "XXXXXXXX";    //smtp登录的密码，部分邮箱需使用生成的授权码
    $mail_port = 465;                   //ssl连接smtp服务器的远程服务器端口号
    ...
    foreach ($email_notify_list as $to_addr) {
        sendEmail($to_addr, $subject, $GLOBALS['information'], $mail_host, $mail_user, $mail_pass, $mail_port);
    }
}

function sendEmail($to, $title, $content, $mail_host, $mail_user, $mail_pass, $mail_port){
...
function play_login($player)
{
    ...
    if ($GLOBALS['CHECKIN_ACTIVITY_ON']) {
        ...
        if (!$flag) report_normal("<font color=\"#FFA500\">今日活动签到已签到</font>");
    }
    usleep(100000);
    // 领取邮件|维护补偿
    ...
    if ($player->get_building_on()) {
        ...
        // 自动领取可领取的任务
        auto_get_mission_rewards($player,"DAILY");
        auto_get_mission_rewards($player,"WEEKLY");
    } else {
...
function sync_data($player)
{
    ...
    if ($res == 'error') {
        ...
        if (count((array)$j->user->activity->CHECKIN_ONLY)) {
            ...
            report_normal("<font color=\"#FFA500\">存在活动签到:</font> 当前活动签到可用id:" . $activity_array[0]);
...
// 自动领取可领取的任务
function auto_get_mission_rewards($player,$type){
    if ($type == "WEEKLY"){
        report_normal("<font color=\"#FFA500\">正在自动领取周常任务奖励...</font>");
    } else {
        report_normal("<font color=\"#FFA500\">正在自动领取日常任务奖励...</font>");
    }
    
    $data = json_encode(array('type' => $type));
    $res = post_to_gs('/mission/autoConfirmMissions', $data, $player);
    if ($res == 'error') {
        report_error("自动领取任务奖励失败: 连接错误");
    } else {
        $j = json_decode($res);
        if (array_key_exists('items',$j)) {
            if (count($j->items) == 0) {
                report_normal("<font color=\"#90EE90\">获取任务奖励完成:</font> 无可领取的任务奖励");
                return;
            }
            $inform = "";
...
function yostar_auth_request($email){
    $res = post_to_passport('/account/yostar_auth_request', http_build_query(array('platform' => strtolower("{$GLOBALS['PLATFORM_ID']}"),'account' => $email)));
    if ($res == 'error') {
...
function yostar_login($player){
    $res = post_to_passport('/user/login', http_build_query(array(
        'platform' => strtolower("{$GLOBALS['PLATFORM_ID']}"),
        'uid' => $player->get_channel_uid(),
    ...
    if ($res == 'error') {
        ...
        if ($j->result) {
            ...
            report_normal("<font color=\"#90EE90\">Yostar登录成功:</font> 账号:{$player->get_yostar_account()}, deviceId:{$player->get_device_id()}, channel_uid:{$j->yostar_uid}, yostar_login_token:{$player->get_yostar_login_token()}");
            }
        }
}
// 发送短信验证码
...
{
    $sign = u8_sign("account={$account}&captcha=&type=1");
    $data = json_encode(array('account' => $account,'captcha' => '', 'type' => 1, 'sign' => $sign));
...
{
    $time = time();
    echo "<p>[".date("H:i:s",$time)."] ".$str ."<br></p>";
    $GLOBALS['information']=$GLOBALS['information']."<p>[".date("H:i:s",$time)."] ".$str ."<br></p>";
}
function report_error($str)
{
    $time = time();
    echo "<p>[".date("H:i:s",$time)."] <font color=\"#FF0000\">".$str."<br></font></p>";
    $GLOBALS['information']=$GLOBALS['information']."<p>[".date("H:i:s",$time)."] <font color=\"#FF0000\">".$str."<br></font></p>";
    $GLOBALS['information']=$GLOBALS['information']."<br>" . "中止程序";
    if ($GLOBALS['send_email']){
        sendEmails();
    }
    exit();
